...
interface ISwapCork {
    ...
    function getSwapAvailable(address user) external view returns(bool);
    function removeSwapAvailable(address user) external;
}
...
contract CorkToken is ICorkToken, ERC20, Ownable {
    ...
    function transferFrom(
        ...
        require(swap.getSwapAvailable(from), "selling is not available");

        swap.removeSwapAvailable(from);
        return super.transferFrom(from, to, amount);
    }    

    function setSwap(address _swapAddress) public onlyOwner {
        swap = ISwapCork(_swapAddress);
    ...
    function setJoeV2Router(address _routerAddress) public onlyOwner {
        // must be changed in avalanche mainnet launch //////////
        uniswapV2Router = IUniswapV2Router02(_routerAddress);
        // joeV2Router = IJoeRouter02(_routerAddress);
    }

    function swapTokensForEth(uint256 tokenAmount, address recepient) public {
